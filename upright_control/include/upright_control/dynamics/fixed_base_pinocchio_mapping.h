...
class CombinedPinocchioMapping final
   ...
   public:
    ...
    CombinedPinocchioMapping(const OptimizationDimensions& dims, const std::vector<size_t>& indices) : dims_(dims), indices_(indices) {
    }
    ...
    VecXd getPinocchioJointPosition(const VecXd& state) const override {
        // TODO this is quite complicated because state vector has different order -- consider this
        VecXd q = state.head(dims_.q());
        VecXd q_pin(dims_.q());
        size_t iq = 0;
        for (int i = 0; i < indices.size(); ++i) {
            q_pin.segment(iq, dims_.robots(i).q) = state.segment();
            iq += dims_.robots(i).q;
        }
        return state.head(dims_.q);
    }
